...
CALRMAP  TITLE 'Glue code to link from C++ to RACF'
         EJECT ,

*---------------------------------------------------------------------*
* Establish an LE environment with correct addressability             *
*---------------------------------------------------------------------*
CALRMAP  CELQPRLG DSASIZE=DSASIZ,     Use calculated size of DSA       *
               BASEREG=NONE,          Use relative addressing          *
               PARMWRDS=3,            This routine needs 3 parms       *
               PARMREG=11,            Location of callers parms        *
               ENTNAME=callRmap,      Special entry point name         *
               PSECT=CALRMAP1         Psect name to use

callRmap ALIAS C'callRmap'
...
CEEWSA   LOCTR
C_WSA64  CATTR DEFLOAD,RMODE(64),PART(CALRMAP1)
...
*---------------------------------------------------------------------*
CALLPROG DS    0H
         LARL  R15,=V(IRRSIM00)       Entry point to racf admin routine
         L     R15,0(0,R15)
         SAM31
